...
def main():
    parser = argparse.ArgumentParser(
        description="bin_path に module_names とそれが依存したファイルを展開する")

    # 引数の設定を記述する
    parser.add_argument("bin", type=str, help="bundleする対象のファイル名を書く")
    parser.add_argument("module_names", type=str,
                        help="bundleしたいモジュール名を書く(複数でもOK)", nargs='*')
    ...
    with open(bin_path) as bin_file:
        for line in bin_file:
            ...
            if match:
                ...
                module_name = line[left + 8:right].replace(' ', '')
